...
| **MCP.so / PulseMCP / mcpservers.org packets** | `config/public/mcp-directory-profile.json` + `docs/submission/*.md` + `docs/assets/sourceharbor-square-icon.png` | site-specific submission packets now exist in-repo, but live submit/read-back still has not happened | submit the packet to each directory and capture the real receipt | submission receipt, pending-review URL, listing URL, or exact blocker |
| **awesome-opencode** | `docs/submission/awesome-opencode.md` | repo-owned listing payload now exists, but the upstream PR/read-back still has not happened | submit the prepared payload through the upstream contribution path | PR URL, issue URL, maintainer feedback, or exact blocker |
