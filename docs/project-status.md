...
| Official MCP Registry listing + metadata template | **ship-now** | a fresh anonymous registry read-back confirms an active `SourceHarbor MCP` entry; exact public version drift now lives in [public-distribution.md](./public-distribution.md), while the durable truth here is that listing existence is proven and newer refresh still depends on publish auth |
| PyPI package surface | **ship-now** | the public `sourceharbor` project is live; exact public version drift now lives in [public-distribution.md](./public-distribution.md), while the durable truth here is that refresh is blocked by missing publish credentials rather than missing package existence |
...
- official-surface public distribution is now split more sharply:
  - PyPI and Official MCP Registry already have live public read-back; the exact current versions and refresh gap are now centralized in [public-distribution.md](./public-distribution.md), and the remaining blocker here is still publish credentials/auth
  - `awesome-opencode` is submitted and waiting on maintainer review
  - `mcp.so` anonymous direct read-back currently returns `Project not found`, so there is still no public listing proof on that route
  - `mcpservers.org` anonymous direct/search read-back still does not prove a live listing for `sourceharbor`, and direct page requests currently return `404 Not Found`
...
| Official MCP Registry + PyPI version refresh | fresh public read-back still shows both live surfaces lagging the repo-controlled package line; [public-distribution.md](./public-distribution.md) is now the canonical place for the exact current version numbers | the repo now has a successful `uv build` receipt for the current package line; the remaining blocker is publish credential / auth availability, not whether the surfaces exist | once the publish credentials exist, publish the intended current package version, refresh the registry entry, and read both surfaces back before repeating a newer live claim |
| MCP.so direct listing | a fresh anonymous direct read-back at `https://mcp.so/server/sourceharbor-mcp` currently returns `Project not found` | the repo-side packet exists, but this public route still does not prove a live listing | capture either a real submit/accept receipt or a later direct-page read-back that shows the listing has appeared |
| mcpservers.org listing | a fresh anonymous direct/search read-back still does not prove a live listing for `sourceharbor`; direct requests currently return `404 Not Found` | the repo-side packet exists, but the public surface still does not show a readable listing proof | capture either the approval/listing URL or a later public read-back that shows the server |
