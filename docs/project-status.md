...
| GitHub repo topics sync | live GitHub description/homepage match the tracked repo profile, but the live topic set is still missing `github-copilot` and `vscode-agent` compared with `config/public/github-profile.json` | the tracked config is already present in-repo; the remaining step is a deliberate repo-settings sync, which is still a GitHub-side write | sync the live repo topics to the tracked profile intent, then rerun `python3 scripts/github/apply_public_profile.py --verify` |
