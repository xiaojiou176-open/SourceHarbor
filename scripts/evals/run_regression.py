...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run deterministic repo-side eval regression against curated fixture responses."
    )
    ...
    for key, floor in baseline["dimensions"].items():
        if float(dimensions.get(key, 0.0)) < float(floor):
            status = "failed"
            failures.append(
                f"dimension {key}={dimensions.get(key, 0.0):.4f} below floor {float(floor):.4f}"
            )
