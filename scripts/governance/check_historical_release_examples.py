...
def main() -> int:
    ...
    tracked = [
        ...
        for line in git_output(
            "ls-files", "artifacts/releases/**/manifest.json", check=False
        ).splitlines()
        if line.strip()
    ]
    errors: list[str] = []
    ...
    for rel in tracked:
        ...
        if str(payload.get("evidence_scope") or "") != "historical-example":
            errors.append(
                f"{rel}: tracked release manifest must set evidence_scope=historical-example"
            )
        ...
        if git_block.get("dirty") is not True:
            errors.append(
                f"{rel}: tracked historical example must keep git.dirty=true to avoid current-proof confusion"
            )
    ...
    if not readme.is_file():
        ...
        for snippet in required_snippets:
            if snippet not in text:
                errors.append(
                    f"artifacts/releases/README.md missing historical-boundary text `{snippet}`"
                )
