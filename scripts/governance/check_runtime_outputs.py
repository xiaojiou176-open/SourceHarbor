...
def main() -> int:
    ...
    if runtime_root.exists():
        for child in runtime_root.iterdir():
            if child.is_dir() and child.name in allowed_subdirs:
                continue
            errors.append(
                f"runtime root contains undeclared direct child: .runtime-cache/{child.name}"
            )
    ...
    if errors:
        ...
        for item in errors:
            print(f"  - {item}")
        print(
            "  - remediation: run `./bin/workspace-hygiene --apply` to remove illegal repo-root/source-tree runtime residue"
        )
        return 1
