...
def main() -> int:
    ...
    if governance.get("status") not in {"pass", "in_progress"}:
        errors.append(
            "governance audit receipt must be `pass` or `in_progress` in newcomer result proof"
        )
    ...
    if not isinstance(blocking_conditions, list) or not all(
        isinstance(item, str) for item in blocking_conditions
    ):
    ...
    if worktree_dirty:
        ...
        if "dirty_worktree" not in blocking_conditions:
            errors.append(
                "dirty worktree must be listed in current_workspace_verdict.blocking_conditions"
            )
    ...
    if (
        strict_receipt.get("status") != "pass"
        and "repo_side_strict_missing_current_receipt" not in blocking_conditions
    ):
        errors.append("missing current strict receipt must be listed as a blocker")

    if (
        newcomer.get("status") == "pass"
        and governance.get("status") == "pass"
        and strict_receipt.get("status") == "pass"
    ):
        expected_status = (
            "partial" if worktree_dirty else ("pass" if current_proof_aligned else "partial")
        )
        ...
        if overall_status not in {"partial", "missing"}:
            errors.append(
                "newcomer result proof must be `partial` or `missing` when only partial repo-side receipts exist"
            )
