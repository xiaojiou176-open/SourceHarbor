...
def _worktree_changes(root) -> list[str]:
...
def main() -> int:
    ...
    worktree_changes = _worktree_changes(root)
    worktree_dirty = bool(worktree_changes)
    errors: list[str] = []
    rows: list[dict[str, object]] = []
    workflow_report_cache: dict[
        str, tuple[dict[str, object] | None, dict[str, dict[str, object]]]
    ] = {}
        ...
        if not source_commit:
            ...
            if name == "external-lane-workflows":
                ...
                for lane in artifact_payload.get("lanes", []):
                    ...
                    if head_alignment == "historical" and lane_state in {
                        "verified",
                        "blocked",
                        "queued",
                        "in_progress",
                ...
                row["workflow_lane_states"] = [
                    {
                        ...
                        "latest_run_matches_current_head": lane.get(
                            "latest_run_matches_current_head"
                        ),
                    }
                    for lane in artifact_payload.get("lanes", [])
            ...
            if workflow_artifact_rel and external_lane and artifact_status in required_statuses:
                ...
                if cached is None:
                    ...
                    workflow_payload = (
                        _load_json_artifact(workflow_path) if workflow_path.is_file() else None
                    )
                    workflow_rows = {
                ...
                if workflow_payload is None:
                    ...
                    head_alignment, latest_head = _workflow_head_alignment(
                        workflow_lane, head_commit
                    )
    ...
    report = {
        ...
        "worktree_state": {
            ...
            "workspace_freshness": "stale" if worktree_dirty else "current",
            "changed_paths_sample": worktree_changes[:20],
            "note": (
                "dirty worktree means commit-aligned artifacts only prove the committed snapshot, not the current uncommitted workspace"
            ),
        },
        "artifacts": rows,
        "errors": errors,
