...
def main() -> int:
    ...
    for item in matrix.get("matrix", []):
        ...
        if verification_status == "verified":
            ...
            if not row_specific_artifacts:
                row_errors.append(
                    "verified blocker row must declare at least one row-specific artifact"
                )
            if missing_artifacts:
                row_errors.append("missing artifacts: " + ", ".join(missing_artifacts))
            if missing_metadata:
                row_errors.append(
                    "artifacts missing runtime metadata: " + ", ".join(missing_metadata)
                )
            if missing_source_commit:
                row_errors.append(
                    "artifacts missing source_commit: " + ", ".join(missing_source_commit)
                )
    ...
    report_path = (
        root / ".runtime-cache" / "reports" / "governance" / "upstream-same-run-cohesion.json"
    )
    write_json_artifact(
        report_path,
        {
