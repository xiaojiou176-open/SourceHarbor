...
def main() -> int:
        ...
        for lock_file in lock_files:
            ...
            for required in ("README.md", "PATCHES.md"):
                if not (vendor_dir / required).is_file():
                    errors.append(
                        f"{vendor_dir.relative_to(root).as_posix()}: missing required vendor file {required}"
                    )
    ...
    for name in sorted(active_vendor_names):
        if name not in registry_active:
            errors.append(
                f"active vendor/fork/patch `{name}` missing from upstream-registry active list"
            )
