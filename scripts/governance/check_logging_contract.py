...
def main() -> int:
    ...
    if sample_generation.returncode != 0:
        detail = (
            sample_generation.stderr.strip() or sample_generation.stdout.strip() or "unknown error"
        )
    ...
    for check in config.get("critical_checks", []):
        ...
        for snippet in check.get("contains", []):
            if snippet not in content:
                errors.append(
                    f"{check['path']}: missing required logging contract snippet `{snippet}`"
                )
    ...
    if set(channels) != required_channels:
        errors.append(
            "logging channels must exactly match: " + ", ".join(sorted(required_channels))
        )
    ...
    sample_targets = {
        channel: root / rel_path for channel, rel_path in config.get("sample_targets", {}).items()
    }
    ...
    for channel, path in sample_targets.items():
        if not path.is_file():
            errors.append(
                f"logging sample for channel `{channel}` missing: {path.relative_to(root).as_posix()}"
            )
            continue
        lines = [line for line in path.read_text(encoding="utf-8").splitlines() if line.strip()]
        if not lines:
            errors.append(
                f"logging sample for channel `{channel}` is empty: {path.relative_to(root).as_posix()}"
            )
            continue
        try:
    ...
    for channel, paths in optional_sample_targets.items():
        if channel not in required_channels:
            errors.append(
                f"logging contract optional_sample_targets contains unknown channel `{channel}`"
            )
            continue
        for path in paths:
            if not path.is_file():
