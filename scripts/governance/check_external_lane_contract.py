    ...
    if not isinstance(lanes, list) or not lanes:
        ...
        for lane in lanes:
            ...
            if artifact and not artifact.startswith(".runtime-cache/"):
                errors.append(
                    f"lane `{lane.get('name', '<unknown>')}` canonical_artifact must live under .runtime-cache/"
                )
            ...
            if remote_artifact and not remote_artifact.startswith(".runtime-cache/"):
                errors.append(
                    f"lane `{lane.get('name', '<unknown>')}` remote_workflow_artifact must live under .runtime-cache/"
                )
            if remote_artifact:
