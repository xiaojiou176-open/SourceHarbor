...
def main() -> int:
    ...
    # The docs surface is intentionally thinner now. If a dedicated dependency
    # governance explainer exists, keep validating its citations; otherwise do
    # not fail-close on a document that the repository no longer treats as a
    ...
    if dependency_doc.is_file():
        ...
        for required in (
            ...
            if required not in dependency_text:
                errors.append(
                    f"docs/reference/dependency-governance.md must cite `{required}` as contract truth source"
                )
    ...
    for rel in HIGH_DRIFT_DOCS:
        ...
        if sum(token in text for token in FORBIDDEN_MANUAL_MIRRORS) > 3:
            errors.append(
                f"{rel} appears to mirror too many governance truth-source filenames directly"
            )
