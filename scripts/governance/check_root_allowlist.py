...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Validate tracked public root entries and tolerated local-private root items."
    )
    parser.add_argument(
    ...
    for item in tracked_entries:
        ...
        if nested_hints and not isinstance(nested_hints, list):
            errors.append(
                f"tracked root allowlist entry `{parent_path}` has non-list nested_boundary_hints"
            )
            continue
        for hint in nested_hints:
            if not isinstance(hint, dict):
                errors.append(
                    f"tracked root allowlist entry `{parent_path}` has invalid nested boundary hint"
                )
                continue
    ...
    for item in tolerated_entries:
        ...
        if (
            args.strict_local_private
            and bool(item.get("must_be_untracked"))
            and git_is_tracked(str(item["path"]), tracked_paths=tracked_paths)
        ):
    ...
    missing_declared = sorted(
        name for name in tracked_allowed if name not in current and not name.startswith(".git")
    )
    if missing_declared:
        print(
    ...
    if errors:
        ...
        for item in errors:
            print(f"  - {item}")
        print(
            "  - remediation: run `./bin/workspace-hygiene --apply` to clear repo-root/source-tree runtime residue before rerunning the gate"
        )
        return 1
