...
from common import current_git_commit, load_governance_json, write_json_artifact
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Verify log correlation completeness across runtime JSONL targets."
    )
    parser.add_argument(
        "--output",
        default=".runtime-cache/reports/governance/log-correlation-completeness.json",
        help="Current runtime report output path.",
    )
    ...
    expected_log_root = str(
        evidence_contract.get("buckets", {}).get("logs", {}).get("path") or ".runtime-cache/logs"
    )
    ...
    for channel, rel_path in config.get("sample_targets", {}).items():
        checked_targets.append({"channel": str(channel), "path": str(rel_path)})
    ...
    for rel_path in ADDITIONAL_APP_CORRELATION_TARGETS:
        checked_targets.append({"channel": "app", "path": rel_path})
        _validate_jsonl_target(
            ROOT / rel_path,
            channel="app",
            minimum_fields=minimum_fields,
    ...
    report = {
        ...
        "expected_log_root": expected_log_root,
        "minimum_common_fields": minimum_fields,
        "channel_required_fields": per_channel,
    ...
    write_json_artifact(
        ROOT / args.output,
        report,
        source_entrypoint="scripts/governance/check_log_correlation_completeness.py",
        verification_scope="log-correlation-completeness",
        source_run_id="log-correlation-completeness",
        freshness_window_hours=24,
        extra={"report_kind": "log-correlation-completeness"},
    )
