...
systemctl list-units 'actions.runner.xiaojiou176.pool-*.service' --all --no-pager || true
echo
...
sudo find '${WORK_ROOT}' -maxdepth 6 \\( -path '*/~/.cache/*' -o -path '*/.cache/*' -o -path '*/.runtime-cache/*' \\) -print | sed -n '1,200p'
echo
...
sudo find '${REPO_ROOT}' -maxdepth 6 \\( -path '*/~/.cache/*' -o -path '*/.cache/*' -o -path '*/.runtime-cache/*' \\) -print | sed -n '1,200p' || true
echo
