...
def _check_dependency_boundaries(errors: list[str]) -> None:
    ...
    for rule in payload.get("python_rules", []):
        ...
        if not scope.exists():
            errors.append(
                f"dependency-boundaries: scope missing `{scope.relative_to(root).as_posix()}`"
            )
    ...
    for rule in payload.get("frontend_rules", []):
        ...
        if not scope.exists():
            errors.append(
                f"dependency-boundaries: frontend scope missing `{scope.relative_to(root).as_posix()}`"
            )
...
def _check_module_ownership(errors: list[str]) -> None:
    ...
    for module in payload.get("modules", []):
        ...
        if not module_path.exists():
            errors.append(
                f"module-ownership: module path missing `{module_path.relative_to(root).as_posix()}`"
            )
        ...
        for key in ("source_paths", "target_paths"):
            for raw in bridge.get(key, []):
                ...
                if (
                    ...
                    and not rendered.startswith(".runtime-cache/")
                ):
                    parts = shlex.split(rendered)
                    if len(parts) >= 2 and not parts[1].startswith("-"):
                        rendered = parts[1]
                candidate = root / rendered[2:] if rendered.startswith("./") else root / rendered
