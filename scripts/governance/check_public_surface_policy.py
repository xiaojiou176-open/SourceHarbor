...
def main() -> int:
    ...
    errors: list[str] = []
    allowed_paths = {
        str(item).strip() for item in payload.get("allowed_tracked_paths", []) if str(item).strip()
    }
    ...
    for pattern in forbidden_patterns:
        ...
        if matches:
            errors.append(
                f"forbidden tracked public artifacts for `{pattern}`: {', '.join(matches[:5])}"
            )
    ...
    for rel in payload.get("required_public_samples", []):
        rel_text = str(rel)
        matching_forbidden = [
            str(pattern) for pattern in forbidden_patterns if fnmatch(rel_text, str(pattern))
        ]
