...
def main() -> int:
    ...
    entries = payload.get("tracked_root_allowlist", []) + payload.get(
        "local_private_root_tolerations", []
    )
    errors: list[str] = []
    forbidden_exact_paths = {str(item) for item in denylist.get("forbidden_exact_paths", [])}
    forbidden_segments = {
        str(item).lower() for item in denylist.get("forbidden_semantic_tokens", [])
    }
    ...
    for item in entries:
        ...
        if path in forbidden_exact_paths:
            errors.append(
                f"root semantic cleanliness: top-level path is explicitly denylisted: {path}"
            )
        if path.startswith(".") and path not in ALLOWED_DOTFILES:
            errors.append(
                f"root semantic cleanliness: undeclared dotfile style path `{path}` is not allowed"
            )
        if "__" in path:
            errors.append(
                f"root semantic cleanliness: double-underscore top-level path is forbidden: {path}"
            )
        if re.search(r"[A-Z]", path) and path not in ALLOWED_ROOT_FILES:
            errors.append(
                f"root semantic cleanliness: top-level path must be lowercase or conventional dotfile: {path}"
            )
