...
def main() -> int:
    ...
    for entry in entries:
        ...
        if integration_surface not in KNOWN_INTEGRATION_SURFACES:
            errors.append(
                f"upstream entry {name} uses unsupported integration_surface `{integration_surface}`"
            )
        how_introduced = str(entry["how_introduced"])
        if how_introduced not in KNOWN_HOW_INTRODUCED:
            errors.append(
                f"upstream entry {name} uses unsupported how_introduced `{how_introduced}`"
            )
        private_coupling_risk = str(entry["private_coupling_risk"])
        if private_coupling_risk not in KNOWN_PRIVATE_COUPLING_RISKS:
            errors.append(
                f"upstream entry {name} uses unsupported private_coupling_risk `{private_coupling_risk}`"
            )
        ...
        if stability_class not in KNOWN_STABILITY_CLASSES:
            errors.append(
                f"upstream entry {name} uses unsupported stability_class `{stability_class}`"
            )
        ...
        if latest_verification_status not in KNOWN_VERIFICATION_STATUSES:
            errors.append(
                f"upstream entry {name} uses unsupported latest_verification_status `{latest_verification_status}`"
            )
        ...
        if not isinstance(latest_verification_age_hours, int) or latest_verification_age_hours < 0:
            errors.append(
                f"upstream entry {name} must declare non-negative latest_verification_age_hours"
            )
        if (
            latest_verification_status == "verified"
            and latest_verification_age_hours > freshness_required_hours
        ):
            errors.append(
        ...
        if kind in {"vendor", "fork", "patch"}:
            lock_files = (
                list((root / "vendor").rglob("UPSTREAM.lock")) if (root / "vendor").exists() else []
            )
            if not lock_files:
                errors.append(
                    f"active upstream `{name}` requires live vendor/fork/patch evidence under vendor/"
                )
        if name not in registry_active:
            errors.append(
                f"active upstream `{name}` missing from config/governance/upstream-registry.json active set"
            )
    ...
    for entry in template_entries:
        ...
        if name not in registry_template:
            errors.append(
                f"template upstream `{name}` missing from config/governance/upstream-registry.json template set"
            )
    ...
    for image in strict_ci_contract.get("service_images", {}).values():
        ...
        if not any(image in source for source in image_sources):
            errors.append(
                f"service image missing from config/governance/active-upstreams.json: {image}"
            )
    ...
    if standard_ref not in image_sources:
        errors.append(
            "strict CI standard image missing from config/governance/active-upstreams.json"
        )

    matrix_entries = matrix.get("matrix", [])
    if len(matrix_entries) < 4:
        errors.append(
            "upstream compatibility matrix must define at least four supported combinations"
        )
