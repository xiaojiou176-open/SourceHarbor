...
def _summary_lane_states(markdown: str) -> dict[str, str]:
    ...
    for match in re.finditer(
        r"^\| `(?P<lane>[^`]+)` \| `(?P<state>[^`]+)` \|", markdown, flags=re.MULTILINE
    ):
...
def main() -> int:
    ...
    if newcomer_report is None:
        ...
        raw_blockers = (newcomer_report.get("current_workspace_verdict") or {}).get(
            "blocking_conditions"
        )
        expected_blockers = (
        ...
        if expected_blockers:
            blocker_line = "- fail-close blockers: " + ", ".join(
                f"`{item}`" for item in expected_blockers
            )
        ...
        if worktree_state.get("dirty") is True:
            if "this page fail-closes to the committed snapshot only" not in summary_text:
                errors.append(
                    "dirty worktree summary must explain that current sub-receipts only prove the committed snapshot"
                )
            if (
                "current workspace verdict above is the fail-close interpretation you must trust"
                not in summary_text
            ):
                errors.append(
                    "dirty worktree summary must restate that repo-side strict pass does not upgrade the current workspace verdict"
                )

    remote_report_path = (
        root / ".runtime-cache" / "reports" / "governance" / "remote-platform-truth.json"
    )
    ...
    required_checks_path = (
        root / ".runtime-cache" / "reports" / "governance" / "remote-required-checks.json"
    )
        ...
        for lane in workflow_payload.get("lanes", []):
            ...
            if head_alignment == "historical":
                if workflow_row_state != "historical":
                    errors.append(
                        f"historical workflow row `{workflow_row_name}` must stay `historical` in summary"
                    )
