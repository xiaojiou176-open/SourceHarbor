...
def main() -> int:
    ...
    for name in secret_ready:
        try:
            ...
            print("[sync-gh-actions-env] updated secret")
        except Exception as exc:  # pragma: no cover
            failures.append(f"secret update failed: {exc}")

    for name in var_ready:
        try:
            run_gh(["variable", "set", name, "--repo", args.repo, "--body", env_merged[name]])
            print("[sync-gh-actions-env] updated var")
        except Exception as exc:  # pragma: no cover
            failures.append(f"var update failed: {exc}")
