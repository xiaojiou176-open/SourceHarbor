...
usage() {
  cat <<'EOF'
Usage: ./scripts/ci/api_real_smoke_local.sh [options]
...
ensure_temporal_server_online() {
  ...
  if [[ "$SOURCE_HARBOR_IN_STANDARD_ENV" != "1" ]]; then
...
mkdir -p "$ROOT_DIR/.runtime-cache/logs/tests" "$ROOT_DIR/.runtime-cache/run"
API_LOG="$ROOT_DIR/.runtime-cache/logs/tests/api-real-smoke-local.log"
WORKER_LOG="$ROOT_DIR/.runtime-cache/logs/tests/api-real-smoke-local-worker.log"
TEMPORAL_LOG="$ROOT_DIR/.runtime-cache/logs/tests/api-real-smoke-local-temporal.log"
STATE_DB_PATH="$ROOT_DIR/.runtime-cache/run/api-real-smoke-local-state.sqlite3"
...
export SOURCE_HARBOR_ALLOW_UNAUTH_WRITE="${SOURCE_HARBOR_ALLOW_UNAUTH_WRITE:-true}"
...
(
  ...
  export SOURCE_HARBOR_API_KEY WEB_ACTION_SESSION_TOKEN
...
if ! (
  cd "$ROOT_DIR" && \
  env -u SOURCE_HARBOR_API_KEY -u WEB_ACTION_SESSION_TOKEN \
