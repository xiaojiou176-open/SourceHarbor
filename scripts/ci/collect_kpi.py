...
def _parse_workflow_topology(files: list[Path]) -> dict[str, Any]:
    ...
    return {
        ...
        "normalize_steps": len(
            re.findall(r"normalize-self-hosted-workspace|Normalize self-hosted workspace", text)
        ),
        ...
        "install_web_deps_steps": len(
            re.findall(r"(npm --prefix apps/web ci|prepare_web_runtime\.sh)", text)
        ),
    }
...
def _parse_run_metrics(repo: str, run_id: str, run_attempt: str, token: str) -> dict[str, Any]:
    ...
    cache_misses = 0
    logs_url = (
        f"https://api.github.com/repos/{repo}/actions/runs/{run_id}/attempts/{run_attempt}/logs"
    )
    try:
        ...
        with zipfile.ZipFile(io.BytesIO(archive)) as zf:
            for name in zf.namelist():
                ...
                cache_hits += len(
                    re.findall(
                        r"Cache restored from key|cache hit occurred on the primary key",
                        content,
                        flags=re.IGNORECASE,
                    )
                )
                cache_misses += len(
                    re.findall(
                        r"Cache not found for input keys|cache miss", content, flags=re.IGNORECASE
                    )
                )
    except Exception:
        pass
...
def _render_markdown(payload: dict[str, Any]) -> str:
    ...
    if topology:
        ...
        lines.append(
            f"| topology.install_web_deps_steps | {topology.get('install_web_deps_steps', 0)} |"
        )
    if artifacts:
...
def main() -> int:
    ...
    if not args.coverage_xml_glob:
        args.coverage_xml_glob = [
            ...
            ".runtime-cache/reports/python/python-coverage.xml",
        ]
    if not args.coverage_summary_glob:
        args.coverage_summary_glob = [".runtime-cache/reports/web-coverage/coverage-summary.json"]
    if not args.mutation_glob:
        args.mutation_glob = [
            ".runtime-cache/reports/mutation/mutmut-cicd-stats.json",
            ".runtime-cache/**/*mutmut*.json",
        ]
    if not args.artifact_glob:
        args.artifact_glob = [".runtime-cache/reports/ci-kpi/inputs/**/*"]
    ...
    if args.repo and args.run_id and args.run_attempt and token:
        try:
            payload["kpi"]["run"] = _parse_run_metrics(
                args.repo, args.run_id, args.run_attempt, token
            )
