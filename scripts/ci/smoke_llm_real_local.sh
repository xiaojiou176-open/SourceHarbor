...
DIAGNOSTICS_JSON=".runtime-cache/reports/tests/pr-llm-real-smoke-result.json"
...
usage() {
  cat <<'EOF'
Usage: scripts/ci/smoke_llm_real_local.sh [options]

Options:
  --api-base-url <url>            API base URL (default: http://127.0.0.1:9000)
  --diagnostics-json <path>       Diagnostics output path (default: .runtime-cache/reports/tests/pr-llm-real-smoke-result.json)
...
if [[ -n "${SOURCE_HARBOR_API_KEY:-}" ]]; then
  request_headers+=(-H "X-API-Key: ${SOURCE_HARBOR_API_KEY}")
  request_headers+=(-H "Authorization: Bearer ${SOURCE_HARBOR_API_KEY}")
  AUTH_MODE="x_api_key"
fi
...
PY
  fail "external gemini probe failed (status=${probe_status}, curl_exit=${probe_curl_exit})"
  exit 1
fi
