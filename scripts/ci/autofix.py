...
def _parse_junit_file(
    path: Path, *, max_bytes: int = DEFAULT_MAX_JUNIT_FILE_BYTES
) -> list[dict[str, Any]]:
...
def _parse_log_file(
    path: Path, *, max_bytes: int = DEFAULT_MAX_LOG_FILE_BYTES
) -> list[dict[str, Any]]:
    findings: list[dict[str, Any]] = []
    if path.stat().st_size > max_bytes:
...
def _expand_inputs(paths: list[str], globs: list[str]) -> list[Path]:
    ...
    for pattern in globs:
        result.extend(
            path
            for path in (Path(match) for match in glob.glob(pattern, recursive=True))
            if path.is_file()
        )
...
def main() -> int:
    ...
    parser.add_argument(
        "--output",
        default=".runtime-cache/reports/autofix/autofix-report.json",
