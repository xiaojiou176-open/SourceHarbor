...
Options:
  ...
  --real-profile            Attach to the configured repo-owned real Chrome instance (local-only, Chromium only)
...
if [[ "$use_real_profile" == "1" ]]; then
  if is_truthy_env "${CI:-}" || is_truthy_env "${GITHUB_ACTIONS:-}"; then
    fail "--real-profile is local-only and must not run under CI/GitHub Actions"
  fi
  [[ "$browser" == "chromium" ]] || fail "--real-profile currently requires --browser chromium"
  eval "$(python3 "$ROOT_DIR/scripts/runtime/resolve_chrome_profile.py" --mode repo-runtime --shell-exports)" || fail "unable to resolve configured repo Chrome runtime"
fi
...
def _build_config() -> SmokeConfig:
    ...
    return SmokeConfig(
        ...
        chrome_cdp_url=os.environ.get("SOURCE_HARBOR_CHROME_CDP_URL", "").strip(),
    )
    ...
    with sync_playwright() as p:
        ...
        try:
            if config.use_real_profile:
                if (
                    not config.chrome_user_data_dir
                    or not config.chrome_profile_dir
                    or not config.chrome_cdp_url
                ):
                    raise AssertionError("real-profile mode requires resolved repo Chrome runtime env")
                browser = p.chromium.connect_over_cdp(config.chrome_cdp_url)
                if not browser.contexts:
                    raise AssertionError(
                        f"repo Chrome instance at {config.chrome_cdp_url} has no attachable context"
                    )
            ...
            diagnostics.update(
                {
                    "browser_mode": "repo-cdp-attach" if config.use_real_profile else "ephemeral",
                    "http_status": status,
