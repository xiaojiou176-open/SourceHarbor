...
def shell_exports(contract: dict[str, Any]) -> str:
    ...
    payload = {
        ...
        "STRICT_CI_DEVCONTAINER_WORKSPACE_FOLDER": str(
            devcontainer.get("workspace_folder", image["workdir"])
        ),
        ...
        "STRICT_CI_PROFILE_GOVERNANCE_PUSH_RELEASE": "1"
        if contract["trigger_policy"]["profile_governance_push_release"]
        else "0",
        "STRICT_CI_LIVE_SMOKE_PUSH_RELEASE": "1"
        if contract["trigger_policy"]["live_smoke_push_release"]
        else "0",
        "STRICT_CI_LIVE_SMOKE_SCHEDULE_CRON": contract["trigger_policy"][
            "live_smoke_schedule_cron"
        ],
    }
    for key, value in service_images.items():
        payload[f"STRICT_CI_SERVICE_IMAGE_{key.upper()}"] = value
    return "\n".join(f"export {key}={shlex.quote(value)}" for key, value in payload.items())
...
def github_outputs(contract: dict[str, Any]) -> str:
    ...
    outputs = {
        ...
        "devcontainer_workspace_folder": str(
            devcontainer.get("workspace_folder", image["workdir"])
        ),
        ...
        "profile_governance_push_release": "true"
        if contract["trigger_policy"]["profile_governance_push_release"]
        else "false",
        "live_smoke_push_release": "true"
        if contract["trigger_policy"]["live_smoke_push_release"]
        else "false",
