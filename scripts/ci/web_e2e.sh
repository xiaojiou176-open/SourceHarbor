...
usage() {
  cat <<'EOF'
Usage: ./scripts/ci/web_e2e.sh [chromium|firefox|webkit]
EOF
}
...
start_temporal() {
  mkdir -p "$ROOT_DIR/.runtime-cache/logs/tests" "$ROOT_DIR/.runtime-cache/reports/tests" "$ROOT_DIR/.runtime-cache/evidence/tests"
  temporal server start-dev --ip 127.0.0.1 --port "$WEB_E2E_TEMPORAL_PORT" > "$ROOT_DIR/.runtime-cache/logs/tests/web-e2e-temporal.log" 2>&1 &
  TEMPORAL_PID="$!"
...
start_api() {
  [[ -n "$DATABASE_URL" ]] || fail "DATABASE_URL is required"
  : > "$ROOT_DIR/.runtime-cache/logs/tests/web-e2e-api.log"
  (
    cd "$ROOT_DIR" && \
    DATABASE_URL="$DATABASE_URL" \
  ...
  ) > "$ROOT_DIR/.runtime-cache/logs/tests/web-e2e-api.log" 2>&1 &
  API_PID="$!"
...
start_worker() {
  [[ -n "$DATABASE_URL" ]] || fail "DATABASE_URL is required"
  : > "$ROOT_DIR/.runtime-cache/logs/tests/web-e2e-worker.log"
  (
    cd "$ROOT_DIR" && \
    DATABASE_URL="$DATABASE_URL" \
    TEMPORAL_TARGET_HOST="127.0.0.1:${WEB_E2E_TEMPORAL_PORT}" \
  ...
  ) > "$ROOT_DIR/.runtime-cache/logs/tests/web-e2e-worker.log" 2>&1 &
  WORKER_PID="$!"
  for _ in $(seq 1 30); do
    if ! kill -0 "$WORKER_PID" >/dev/null 2>&1; then
      tail -n 80 "$ROOT_DIR/.runtime-cache/logs/tests/web-e2e-worker.log" >&2 || true
...
run_web_e2e_pytest() {
  ...
  (
    ...
    uv run --with pytest --with playwright \
      ...
      -q --junitxml=".runtime-cache/reports/tests/web-e2e-junit-${BROWSER}.xml" -rA \
      --web-e2e-browser "$BROWSER" \
      --web-e2e-api-base-url "http://127.0.0.1:${WEB_E2E_API_PORT}" \
      2>&1 | tee ".runtime-cache/logs/tests/web-e2e-${BROWSER}.log"
  )
}
...
report = root_dir / ".runtime-cache" / "reports" / "tests" / f"web-e2e-junit-{browser}.xml"
...
if [[ "${STRICT_CI_BOOTSTRAP_RUNTIME_READY:-0}" != "1" ]]; then
  bash "$ROOT_DIR/scripts/governance/runner_workspace_maintenance.sh" --workspace "$ROOT_DIR"
  source "$ROOT_DIR/scripts/ci/bootstrap_strict_ci_runtime.sh"
fi
eval "$(bash "$ROOT_DIR/scripts/ci/prepare_web_runtime.sh" --shell-exports)"
export WEB_E2E_RUNTIME_WEB_DIR="$WEB_RUNTIME_WEB_DIR"
export SOURCE_HARBOR_REPO_ROOT="$ROOT_DIR"
