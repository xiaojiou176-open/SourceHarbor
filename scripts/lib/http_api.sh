...
resolve_http_api_base_url() {
  ...
  if declare -F resolve_runtime_route_value >/dev/null 2>&1 && [[ -n "$root_dir" ]]; then
    resolve_runtime_route_value "$root_dir" "SOURCE_HARBOR_API_BASE_URL" "" "$default_value"
    return 0
  fi

  if [[ -n "${SOURCE_HARBOR_API_BASE_URL:-}" ]]; then
...
apply_http_api_base_url() {
  local cli_value="${1:-}"
  local root_dir="${2:-${ROOT_DIR:-}}"
  export SOURCE_HARBOR_API_BASE_URL
  SOURCE_HARBOR_API_BASE_URL="$(resolve_http_api_base_url "$cli_value" "$root_dir")"
  export HTTP_API_BASE_URL_OVERRIDE="$SOURCE_HARBOR_API_BASE_URL"
}

http_api_auth_args() {
  local -a args=()
  if [[ -n "${SOURCE_HARBOR_API_KEY:-}" ]]; then
    args+=(-H "X-API-Key: ${SOURCE_HARBOR_API_KEY}")
  fi
...
api_get() {
  ...
  base_url="$(resolve_http_api_base_url "${HTTP_API_BASE_URL_OVERRIDE:-${SOURCE_HARBOR_API_BASE_URL:-}}" "${ROOT_DIR:-}")"
  local -a auth_args=()
  local auth_part
...
check_api_health() {
  ...
  http_api_log "API reachable: $(resolve_http_api_base_url "${HTTP_API_BASE_URL_OVERRIDE:-${SOURCE_HARBOR_API_BASE_URL:-}}" "${ROOT_DIR:-}")"
}
