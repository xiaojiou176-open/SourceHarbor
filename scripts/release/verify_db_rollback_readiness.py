...
def _validate_drill_evidence(path: Path) -> tuple[bool, list[str]]:
    ...
    errors: list[str] = []
    for key in ("release_tag", "executed_at", "executor", "strategy", "result"):
        value = str(payload.get(key) or "").strip()
        if not value:
            errors.append(f"{key} must be non-empty")
            continue
        if key == "result" and value == "pending":
            errors.append("result must not be `pending`")
    migrations_checked = payload.get("migrations_checked")
    if not isinstance(migrations_checked, list):
        errors.append("migrations_checked must be a list")
    elif not migrations_checked:
        errors.append("migrations_checked must record at least one migration")
...
def main() -> int:
    ...
    gate_status = (
        "pass" if missing_policy == 0 and with_blocker == 0 and invalid_down_sql == 0 else "fail"
    )
    payload = {
